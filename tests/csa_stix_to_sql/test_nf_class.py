...
class TestStixToSql(unittest.TestCase, object):
    ...
    def test_ipv4_query(self):
        ...
        where_statement = "WHERE (Network.A = '192.168.122.84' OR Network.B = '192.168.122.84') OR (Network.A = '192.168.122.83' OR Network.B = '192.168.122.83') PARTITIONED EVERY 10 ROWS"
    ...
    def test_ipv4_in_query(self):
        ...
        options = {}
        query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE (Network.A IN (192.168.122.83 OR 192.168.122.84) OR Network.B IN (192.168.122.83 OR 192.168.122.84)) PARTITIONED EVERY 10 ROWS"
#        parsed_stix = [{'attribute': 'ipv4-addr:value', 'comparison_operator': 'IN', 'value': '192.168.122.84'}, {'attribute': 'ipv4-addr:value', 'comparison_operator': 'IN', 'value': '192.168.122.83'}]
    ...
    def test_ipv6_query(self):
        ...
        options = {}
        query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE (Network.A = '192.168.122.83' OR Network.B = '192.168.122.83') PARTITIONED EVERY 10 ROWS"
    ...
    def test_mac_address_query(self):
        ...
        options = {}
        query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE (Link.A = '00-00-5E-00-53-00' OR Link.B = '00-00-5E-00-53-00') PARTITIONED EVERY 10 ROWS"
    ...
    def test_query_from_multiple_observation_expressions_joined_by_and(self):
        interface = csa_translator.Translator(dialect='nf', rows=10)
        input_arguments = "[ipv4-addr:value = '192.168.1.1'] and [mac-addr:value = '00-00-5E-00-53-00']"
        options = {}
        query = interface.transform_query(input_arguments, options)
        # Expect the STIX and to convert to an SQL OR.
        where_statement = "WHERE (Network.A = '192.168.1.1' OR Network.B = '192.168.1.1') OR (Link.A = '00-00-5E-00-53-00' OR Link.B = '00-00-5E-00-53-00') PARTITIONED EVERY 10 ROWS"
        parsed_stix = [{'attribute': 'ipv4-addr:value', 'comparison_operator': '=', 'value': '192.168.1.1'}, {'attribute': 'mac-addr:value', 'comparison_operator': '=', 'value': '00-00-5E-00-53-00'}]
        assert query == {'sql_queries': [selections + from_statement + where_statement], 'parsed_stix': parsed_stix}
    ...
    def test_query_from_multiple_comparison_expressions_joined_by_and(self):
        ...
        where_statement = "WHERE (Link.A = '00-00-5E-00-53-00' OR Link.B = '00-00-5E-00-53-00') AND (Network.A = '192.168.1.1' OR Network.B = '192.168.1.1') PARTITIONED EVERY 10 ROWS"
        parsed_stix = [{'attribute': 'mac-addr:value', 'comparison_operator': '=', 'value': '00-00-5E-00-53-00'}, {'attribute': 'ipv4-addr:value', 'comparison_operator': '=', 'value': '192.168.1.1'}]
        assert query == {'sql_queries': [selections + from_statement + where_statement], 'parsed_stix': parsed_stix}
    ...
    def test_port_queries(self):
        ...
        options = {}
        query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE Transport.B = '23456' OR Transport.A = '12345' PARTITIONED EVERY 10 ROWS"
    ...
    def test_network_traffic_protocols(self):
        ...
        for key, value in protocols.items():
            ...
            options = {}
            query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE Transport.Protocol = '" + value + "' PARTITIONED EVERY 10 ROWS"
    ...
    def test_network_traffic_start_stop(self):
        ...
        options = {}
        query = interface.transform_query(input_arguments, options)
        where_statement = "WHERE Last = '1528965384' OR Start = '1528965384' PARTITIONED EVERY 10 ROWS"
